...
export function BaseNode({ className, status, ...props }: BaseNodeProps) {
  return (
    ...
    >
      ...
      {status === "error" && (
        <XCircleIcon className="absolute right-1.5 bottom-1.5 size-2 text-red-700 stroke-3" />
      )}

      {status === "success" && (
        <CheckCircle2 className="absolute right-1.5 bottom-1.5 size-2 text-green-700 stroke-3" />
      )}
